...
for i = 1:amountSources
    m = (i-1)*2+1;
    Z_t = t(1,i)-(sqrt(((X-double(coordinate_set(m)))*dx).^2+((Y-double(coordinate_set(m+1)))*dy).^2)/340);
    Z_t = -Z_t;
    Z = Z+Z_t;
end
...
highestMask = ones(Nx,Ny)*5;
highestMask(Mx,My) = 10;

originalMask = zeros(Nx,Ny);
originalMask(x_o,y_o) = 10;
figure;
subplot(1,3,1)
